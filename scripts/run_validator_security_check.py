...
def main() -> None:
    ...
    run([sys.executable, "scripts/collect_created_bank_state_accounts.py", str(out), "--pause", "0.35", "--retries", "5"])
    run([sys.executable, "scripts/analyze_created_bank_state_accounts.py", str(out)], out / "created-bank-state-account-correlation.md")
    run([sys.executable, "scripts/analyze_bankk_41_byte_state.py", str(out)], out / "bankk-41-byte-state-layout.md")
    run([sys.executable, "scripts/analyze_bankk_local_id_lifecycle.py", str(out)], out / "bankk-local-id-lifecycle.md")
    ...
    print(f"Created Bank state account correlation: {out / 'created-bank-state-account-correlation.md'}")
    print(f"BankK 41-byte state layout: {out / 'bankk-41-byte-state-layout.md'}")
    print(f"BankK local ID lifecycle: {out / 'bankk-local-id-lifecycle.md'}")
