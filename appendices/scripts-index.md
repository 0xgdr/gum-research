...
Derives root-update submitters from decoded outbox root updates, then fetches a bounded Solana mainnet signature window, transaction bodies and current account state for each submitter.
...
Analyzes direct root submitter history for invoked programs, root-update cadence, lamport deltas, token-balance hints, parsed transfers and intersections with canonical JUP, validator/vote/stake keys or parsed upgrade authorities.
...
Pages further back through root submitter signatures after the direct-history window, fetching older Solana mainnet transaction bodies until the configured limit or first positive lamport-delta event.

Example:

```bash
python3 scripts/collect_root_submitter_funding_history.py \
  evidence/YYYY-MM-DD-live-rpc \
  --signature-limit 300 \
  --transaction-limit 300 \
  --stop-after-positive 1
```

### `scripts/analyze_root_submitter_funding_history.py`

Analyzes older root submitter transactions for funding/setup provenance, including positive lamport deltas, transfer sources, Gum Bank request involvement, token-balance hints and intersections with canonical JUP, validator/vote/stake keys or parsed upgrade authorities.

Example:

```bash
python3 scripts/analyze_root_submitter_funding_history.py \
  evidence/YYYY-MM-DD-live-rpc
```
...
Collects a bounded Solana mainnet transaction cohort for a Gum Bank withdrawal-related address. By default it targets `bk1PDA...`, the Gum Bank request path that funded the root submitter setup event. Use `--address` and `--output-prefix` to collect comparable cohorts such as the `BankK...` Bank Program surface without overwriting the default files.
...
python3 scripts/collect_bank_withdrawal_cohort.py \
  ...
  --output-prefix bank-program-withdrawal-cohort \
  --signature-limit 50 \
  --transaction-limit 50
```
...
Decodes `bk1PDA...` Request/Withdraw rows, compares recipients/mints/implementation programs/signers against the root-submitter setup event and reports whether any cohort row exposes canonical JUP, validator, vote or stake material.

Example:

```bash
python3 scripts/analyze_bank_withdrawal_cohort.py \
  evidence/YYYY-MM-DD-live-rpc
```

### `scripts/analyze_withdrawal_surface_comparison.py`

Compares the `bk1PDA...`, `BankK...` and root-submitter setup surfaces. It reports decoded recipients, implementation programs, helper programs, mints, fee payers, signers, inbox/outbox behavior and canonical JUP / current validator / vote / stake intersections.

Example:

```bash
python3 scripts/analyze_withdrawal_surface_comparison.py \
  evidence/YYYY-MM-DD-live-rpc
```

### `scripts/analyze_bank_request_message_correlation.py`

Tests whether decoded `bk1PDA...` withdrawal requests can be joined to sampled `BankK...` rows by message hash, withdrawal-request pubkey, `jupnet` pubkey, recipient account, token transfer or raw payload bytes. It also summarizes the `BankK...` instruction role map and created account layouts.

Example:

```bash
python3 scripts/analyze_bank_request_message_correlation.py \
  evidence/YYYY-MM-DD-live-rpc
```

### `scripts/collect_created_bank_state_accounts.py`

Derives Bank/Gum accounts created by the sampled `bk1PDA...`, `BankK...` and root-submitter setup transactions, then fetches their current Solana account state with `getMultipleAccounts`.
...
Analyzes current state for the created Bank/Gum accounts. It classifies closed accounts, `bk1PDA...` request state, `BankK...` state and SPL token accounts, then scans account bytes for decoded request/message/recipient fields, canonical JUP and current validator/vote/stake key material.
...
### `scripts/analyze_bankk_41_byte_state.py`

Decodes compact 41-byte `BankK...` state accounts as discriminator, flag and embedded 32-byte id/pubkey candidate. It correlates the embedded value against sampled transaction account keys, `BankK...` / `JNiN...` raw payloads, decoded `bk1PDA...` request fields, verifier/root fields and canonical JUP/current validator/vote/stake keys.

Example:

```bash
python3 scripts/analyze_bankk_41_byte_state.py \
  evidence/YYYY-MM-DD-live-rpc
```

### `scripts/analyze_bankk_local_id_lifecycle.py`

Traces 41-byte `BankK...` local ids across sampled transaction lifecycles. It reports operation, inbox-helper and `VerifyRequest` appearances, same-slot operation/verify pairings, root-update slot proximity and intersections with decoded `bk1PDA...`, verifier/root, canonical JUP and current validator/vote/stake material.
...
Runs the full monitoring workflow: collect a fresh snapshot, fetch recurring Bank account state, fetch owner-program context, fetch JupNet helper-program-owned accounts, fetch outbox root-update transactions, fetch wider outbox root history, fetch root submitter direct history, fetch root submitter funding history, classify funding actors, collect/analyze the Bank withdrawal cohort, collect the comparable `BankK...` withdrawal cohort, compare withdrawal surfaces, correlate request/message evidence, fetch/analyze current state for created Bank/Gum accounts, decode compact `BankK...` 41-byte state, trace `BankK...` local-id lifecycles, fetch the Gum omnichain sender program, fetch all visible JupNet executable ProgramData accounts, generate `analysis.md`, generate `deep-dive.md`, generate `authorization.md`, generate `utility-classification.md`, generate `solana-bank.md`, generate `bank-reverse-engineering.md`, generate `bank-account-graph.md`, generate `bank-recurring-account-state.md`, generate `bank-owner-program-context.md`, generate `jupnet-helper-program-accounts.md`, generate `verify-request-payload-reconstruction.md`, generate `outbox-root-update-transactions.md`, generate `outbox-update-payload-reconstruction.md`, generate `outbox-root-history.md`, generate `root-update-authority-graph.md`, generate `root-submitter-provenance.md`, generate `root-submitter-history.md`, generate `root-submitter-funding-history.md`, generate `funding-actor-classifier.md`, generate `bank-withdrawal-cohort.md`, generate `withdrawal-surface-comparison.md`, generate `bank-request-message-correlation.md`, generate `created-bank-state-account-correlation.md`, generate `bankk-41-byte-state-layout.md`, generate `bankk-local-id-lifecycle.md`, generate `epoch-security-source-hunt.md`, generate `outbox-verifier-payload-field-map.md`, generate `security-boundary-corpus.md`, generate `private-runtime-fingerprints.md`, generate `gum-omnichain-binary-roles.md`, generate `gum-account-role-reconstruction.md`, generate `gum-omnichain-sender-program.md`, generate `jupnet-executable-census.md`, and compare against the latest prior snapshot when available.
