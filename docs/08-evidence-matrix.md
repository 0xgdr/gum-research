...
| Root-update authority path is publicly observable | Confirmed | The decoded root update was submitted by transaction signer `6f5mu...`, invoked the outbox helper, verified BLS/Merkle logs and wrote the outbox root-history account |
| Root-update authority graph exposes JUP/Dove/security producer state | Not observed | Root-update signer/accounts had zero canonical JUP/current validator/vote/stake intersections and did not match parsed Gum/Bank/inbox/outbox upgrade authorities |
| Root-update submitter appears elsewhere in the saved corpus | Not observed | `6f5mu...` appeared in one saved transaction body, only as the root-update signer/payer |
| Root-update submitter exposes funding or token utility clues | Not observed | The only observed balance change was `-5000` lamports matching the transaction fee; no token-balance hints appeared |
| Root-update submitter is a dedicated root publisher in the fetched window | Strong evidence | 30 direct history transactions from 2026-06-30 to 2026-07-13 all invoked only `jnoUtn...` and logged successful `UpdateMerkleRoot` BLS/Merkle verification |
| Direct root submitter history exposes JUP/Dove/security producer state | Not observed | Direct history had zero canonical JUP/current validator/vote/stake intersections, zero parsed upgrade-authority intersections and no token-balance hints |
| Root submitter funding/setup event is observable | Confirmed | Funding-history walk found one positive `6f5mu...` lamport delta on 2026-03-17, crediting `2132273211` lamports |
| Root submitter funding/setup ties to Gum Bank workflow | Strong evidence | The positive funding transaction invoked `bk1PDA...`, logged `Instruction: Request`, called `op16...` `Withdraw`, emitted `jupnet`, `message_hash` and `Signature verified` logs, and wrote request-account data |
| Root submitter funding payload decodes to concrete recipient/amount/program fields | Confirmed | Funding actor classifier decoded wrapped SOL mint, recipient `6f5mu...`, amount `2132273211`, implementation program `op16...` and matching withdrawal request/JupNet bytes |
| Funding-side fee payer/signer is observable | Confirmed | `JUPW3...` is a system-owned non-executable account, fee payer and signer in the setup transaction, and signer in 26 local transaction files |
| Root submitter setup used a reusable withdrawal implementation | Confirmed | Bank withdrawal cohort found `op16...` in all 19 decoded withdrawal-like rows, including the setup comparison row |
| Root submitter appears as one withdrawal recipient among many | Strong evidence | The `bk1PDA...` cohort plus setup comparison produced 15 unique decoded recipients; `6f5mu...` appeared once |
| Root submitter setup is distinct by mint in the compared cohort | Confirmed | The 18 decoded cohort Request/Withdraw rows used USDC, while the root submitter setup comparison used wrapped SOL |
| `bk1PDA...` and `BankK...` expose connected Solana-side Gum withdrawal layers | Strong evidence | Withdrawal surface comparison shows `bk1PDA...` decoded Request/Withdraw rows through `op16...`, while `BankK...` rows invoke inbox helper `JNiN...` and outbox helper `jnoUtn...`; `JUPW3...` signs/pays both sampled surfaces |
| Withdrawal-surface comparison exposes JUP/Dove/security producer state | Not observed | The compared `bk1PDA...`, `BankK...` and root-submitter setup rows had zero canonical JUP/current validator/vote/stake intersections and no visible signer-set, quorum, weight, slashing or reward state |
| A decoded `bk1PDA...` request can be joined to sampled `BankK...` rows by public per-message identifiers | Not observed | Bank request/message correlation found zero decoded message-hash hits, zero withdrawal-request/`jupnet` pubkey hits, zero recipient hits and zero token near-matches inside sampled `BankK...` account keys or raw payload blobs |
| `BankK...` and `bk1PDA...` expose different public state layouts | Strong evidence | Correlation analysis found decoded `bk1PDA...` request rows creating 72-byte request accounts while `BankK...` Withdraw/VerifyRequest rows created 41-byte and 223/256-byte Bank-owned accounts |
| Current `bk1PDA...` request accounts preserve decoded request state | Confirmed | Created account-state analysis found current 72-byte `bk1PDA...` accounts retaining `op16...` at offset 8 and some request/`jupnet` pubkeys at offset 40 |
| Current `BankK...` state accounts preserve decoded `bk1PDA...` join identifiers | Not observed | Created account-state analysis found zero decoded `bk1PDA...` message-hash, request/`jupnet` pubkey or recipient hits inside current `BankK...` state accounts |
| Created Bank/Gum state exposes JUP/Dove/security producer state | Not observed | Created account-state analysis found zero canonical JUP/current validator/vote/stake key hits across 85 created accounts |
| Current `BankK...` 41-byte state preserves a Bank-local message/state id | Confirmed | Layout analysis decoded 19 records as discriminator `d201f3aeaed73c91`, flag `1` and a unique 32-byte id; 18 ids reappeared in sampled `BankK...` / `JNiN...` raw payloads |
| `BankK...` 41-byte state exposes JUP/Dove/security producer state | Not observed | The embedded 32-byte ids matched zero decoded `bk1PDA...` fields, verifier/root fields, canonical JUP keys or current validator/vote/stake keys |
| `BankK...` local ids bridge operation and verify payloads | Strong evidence | Lifecycle analysis found 18 local ids in both Bank operation payloads and `VerifyRequest` payloads, including 12 same-slot operation/verify pairings |
| `BankK...` local ids bridge to outbox roots or JUP/Dove/security producer state | Not observed | Lifecycle analysis found zero local ids in `jnoUtn...` outbox raw payloads, zero root-update same-slot hits, zero verifier/root field matches and zero canonical JUP/current validator/vote/stake matches |
| Wider `BankK...` window preserves Bank-local operation/verify lifecycle | Strong evidence | 200 transactions yielded 194 analyzed local ids, 175 payload-derived ids, 69 operation/verify lifecycle bridges and 52 same-slot operation/verify bridges |
| Wider `BankK...` window exposes JUP/Dove/security producer state | Not observed | The widened lifecycle analysis found zero `jnoUtn...` outbox payload hits, zero verifier/root hits, zero decoded `bk1PDA...` request-field hits, zero canonical JUP/current validator/vote/stake hits and zero root-update-slot hits |
| Root submitter funding exposes JUP/Dove/security producer state | Not observed | The funding-history window had zero canonical JUP/current validator/vote/stake intersections and zero parsed upgrade-authority intersections |
...
| Gum omnichain binaries have distinct public roles | Strong evidence | `brhPf...` exposes BN254/cross-chain-hash/alt-BN128 BLS symbols while `GUMeb...` is the sender/program id recovered from verifier payloads and exposes Gum omnichain instruction/state paths |
| Gum omnichain binary-role analysis exposes JUP security producer state | Not observed | Neither binary exposed stake, validator, vote, Dove, quorum, weight, slashing, reward, signer-set or root-builder implementation strings |
| Gum account roles can be reconstructed from sampled metas | Confirmed | 8 direct `brhPf...` Gum instructions, 17 Solana Bank instructions and 11 `GUMeb...` verifier sender observations were mapped to admin/config, request, token, inbox and outbox proof roles |
| Gum account-role reconstruction exposes JUP/Dove/security producer state | Not observed | Account metas had zero canonical JUP hits and zero current validator/vote/stake hits; no signer-set, quorum or weight role candidate appeared |
