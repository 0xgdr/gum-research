...
- `BankK...` 41-byte state layout analysis found a stable compact record: discriminator `d201f3aeaed73c91`, flag byte `1`, and a 32-byte Bank-local id that reappears in sampled `BankK...` / `JNiN...` inbox payloads while still matching zero decoded `bk1PDA...` fields, verifier/root fields, canonical JUP or current validator/vote/stake keys.
- Bank-local id lifecycle analysis found 18 ids that bridge `BankK...` operation payloads and `VerifyRequest` payloads, including 12 same-slot operation/verify pairings, but none crossed into decoded `bk1PDA...`, outbox/root, canonical JUP or validator/vote/stake material.
- A wider 200-transaction `BankK...` lifecycle window recovered 175 payload-derived local ids; 69 bridged operation plus verify evidence and 52 bridged in the same slot, while still producing zero outbox, verifier/root, decoded `bk1PDA...`, canonical JUP, validator, vote, stake or root-update-slot hits.
...
- **BankK 41-byte state:** current compact `BankK...` state does preserve a Bank-local 32-byte operation id that repeats in `BankK...` and `JNiN...` payloads. This improves the Bank-local linkage model but still does not expose the missing `bk1PDA...` join or Dove/JUP/stake producer state.
- **BankK local-id lifecycle:** 18 local ids link Bank operation payloads to `VerifyRequest` payloads, and 12 do so in the same slot. The public lifecycle is now visible inside the Bank layer, but it still stops before decoded `bk1PDA...` request fields, outbox roots and Dove/JUP/stake producer state.
- **BankK wide-window lifecycle:** a contiguous 200-transaction `BankK...` window strengthens the Bank-local lifecycle evidence with 69 operation/verify local-id bridges and 52 same-slot bridges. The wider sample still stops at the same boundary: no outbox payload hits, verifier/root hits, decoded `bk1PDA...` joins, canonical JUP hits, validator/vote/stake hits or root-update-slot hits.
